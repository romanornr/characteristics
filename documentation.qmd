...
```{r}
baseplot <- ggplot(all_patients, aes(x = Age, y = GMV, color = group)) +
  geom_point(aes(shape = as.factor(Scannertype))) +
  geom_point(dmd_patients_first_visit, aes(x = Age, y = GMV, color = "DMD")) +
  geom_smooth(method = "lm", se = TRUE, aes(fill = group)) +
  scale_shape_discrete(name = "Scanner Type") +  # Separate legend for shape
  scale_color_discrete(name = "Group") +         # Separate legend for color
  scale_fill_discrete(name = "Group") +          # Separate legend for fill
  labs(title = "Gray Matter Volume vs Age", x = "Age", y = "GMV", color = "Group", shape = "Scanner Type") +
  theme_bw()
